...
function createMapLayoutScreen(language) {
    const MapLayout = {
        player: null,
        isDrawn: false,
        next: null,
        ...
        currentShipIndex: 0,
        isHorizontal: false,
        map: create2DArrayWithFill(GAME_BOARD_DIM, GAME_BOARD_DIM, 0), // Updated to specify rows and columns
            ...
            if (KeyBoardManager.isEnterPressed() && this.currentShipIndex < this.ships.length) {
                ...
                if (this.canPlaceShip()) {
                    ...
                    if (this.currentShipIndex >= this.ships.length) {
                        // Make a deep copy of this.map before passing it
                        const playerMapCopy = JSON.parse(JSON.stringify(this.map));
                        this.next = this.transitionFn(playerMapCopy);
        ...
        draw: function (dr) {
            ...
            let output = `${ANSI.TEXT.BOLD}${ANSI.COLOR.YELLOW}${languages[language].ship_placement_phase}\n\n${ANSI.TEXT.BOLD_OFF}${ANSI.RESET}`;
            output += '  ';
            ...
            output += `${languages[language].r_rotate}\n`;
            output += `${languages[language].enter_place_ship}\n`;

            output += `\n${ANSI.TEXT.BOLD}${ANSI.COLOR.YELLOW}${languages[language].ships_to_place}${ANSI.TEXT.BOLD_OFF}${ANSI.RESET}\n`;
